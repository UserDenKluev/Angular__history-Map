...
export class AllSiteService {
  ...
  getCountry() {
    this.http.get<Country[]>('https://server-history.onrender.com/countries').subscribe((res) => {
      for (let index = 0; index < res.length; index++) {
        this.countries[index] = res[index]
      }
      for (let index = 0; index < res.length; index++) {
        this.saveCountries[index] = res[index]
      }
  ...
  getEvents() {
    this.http.get<Event[]>('https://server-history.onrender.com/events').subscribe((res) => {
      for (let index = 0; index < res.length; index++) {
        this.events[index] = res[index]
      }
      for (let index = 0; index < res.length; index++) {
        this.saveEvents[index] = res[index]
      }
      this.getYearOrder();
  ...
  getUsers() {
    this.http.get<Users[]>('https://server-history.onrender.com/users').subscribe((res) => {
      for (let index = 0; index < res.length; index++) {
  ...
  getOffrs() {
    this.http.get<Offer[]>('https://server-history.onrender.com/offer').subscribe((res) => {
      for (let index = 0; index < res.length; index++) {
        this.offers[index] = res[index]
      }
    })
  }
  ...
  addCountry(data: Country) {
    ...
    this.http.post<Country>('https://server-history.onrender.com/countries', data)
    .subscribe((res) => {
      console.log('второй');
  ...
  addEvent(data: Event) {
    this.http.post<Event>('https://server-history.onrender.com/events', data)
    .subscribe((res) => {
  ...
  putCountry(country) {
    ...
    this.http.put(`https://server-history.onrender.com/countries/${country.id}`, {
      id: country.id,
      name: country.name,
      color: country.color,
  ...
  putEventWithCountry(idEvent: number) {
    this.http.put(`https://server-history.onrender.com/events/${idEvent}`, {
  ...
  putEvent(event) {
    this.http.put(`https://server-history.onrender.com/events/${event.id}`, {
  ...
  deleteCountry(id: number) {
    this.http.delete(`https://server-history.onrender.com/countries/${id}`)
    .subscribe(() => {
      let eventId = 0;
      for (let idCountry = 0; idCountry < this.saveCountries.length; idCountry++) {
  ...
  deleteEventWithCountry(idEvent: number) {
    this.http.delete(`https://server-history.onrender.com/events/${idEvent}`)
    .subscribe(()=>{
      for (let i = 0; i < this.saveEvents.length; i++) {
  ...
  addUser(data: Users) {
    this.http.post<Users>('https://server-history.onrender.com/users', data)
    .subscribe((res) => {
  ...
  addOffer(data: Offer) {
    this.http.post<Offer>('https://server-history.onrender.com/offer', data)
    .subscribe((res) => {
  ...
  deleteOffer(id: number, iMas: number) {
    this.http.delete(`https://server-history.onrender.com/offer/${id}`)
    .subscribe(() => {
